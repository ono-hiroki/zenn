...
RSA暗号の解説でよく見る「e」や「d」という記号。これらが何を意味していて、なぜ暗号化したものが復号で元に戻るのか、数学的な原理から理解してみましょう。
...
暗号化は「m を e 回かけて、n で割った余り」です。この余りが暗号文 c（ciphertextの略）になります。

```
c = m^e mod n
```

復号は「c を d 回かけて、n で割った余り」です。すると元の平文 m が出てきます。
...
つまり「m を e 回かけて、n で割った余り」が暗号文です。
...
つまり「c を d 回かけて、n で割った余り」が元の平文です。
...
> m と n が互いに素のとき、m を φ(n) 回かけて n で割ると、余りは 1 になる
...
ステップ4と5で、e と d は拡張ユークリッドの互除法で求まり、ベズーの等式により存在が保証されることを説明しました。ここでは具体的な計算方法を見ていきます。
