...
しかし、この対応関係は**必須ではありません**。

- 同じテーブルを参照するEntityが**複数あってもいい**
- 複数のテーブルを**1つのEntityにまとめてもいい**
...
巨大なEntityを変更するとき、「この変更は他のどこに影響するか？」の判断が難しくなります。
...
これらは独立した関心事です。認証の仕様変更とプロフィールの仕様変更は、通常は別々に起こります。
...
「認証できるか？」と「名前は何か？」は異なる関心事です。

これらを同じEntityに混在させると、一方の変更が他方に影響するリスクが生まれます。関心事ごとにEntityを分ければ、変更の影響範囲が明確になります。
...
しかし、`passwordHash`と`name`の関連性は低いです。これらを同じEntityに入れると、凝集度が下がります。
...
同時に作成・更新・削除されるデータは、同じEntityで管理する方が自然です。
...
変更理由が複数あり、それぞれが独立しているなら、分割の候補です。

### 分割を検討するケース：利用する場面が異なる

同じデータでも、使われる場面が異なるなら、分割を検討します。

- 認証処理では「ログインID、パスワード、ロック状態」
- プロフィール表示では「名前、メール、アイコン」
- 権限チェックでは「ロール、権限リスト」

それぞれの場面で必要なプロパティだけを持つEntityを作れば、シンプルで扱いやすくなります。
...
これらは全て同じ`users`テーブルを参照しますが、**それぞれが単一の責任を持つ小さなEntity**です。
...
| ビジネスルール | プロパティ間に守るべきルールがあるなら | ルールがないなら |
| ライフサイクル | 同時に作成・削除されるなら（テーブルが別でも） | 独立して存在するなら |
| 利用場面 | 常に一緒に使うなら | 場面ごとに異なるなら |

**Entityはテーブルの写像ではなく、特定の場面における責務の表現です。**

テーブルとEntityの対応関係にこだわる必要はありません。設計原則に従って、統合すべきものは統合し、分割すべきものは分割する。基本原則に従った設計が、長期的にメンテナンスしやすいコードにつながります。
