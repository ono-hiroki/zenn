---
title: "「依存関係は正しいけど、DTOがあったほうがいい」という話"
emoji: "🧱"
...
Presentation層（Controller）がDomain層（Entity）を参照する。UseCase層もEntityを参照する。依存の矢印は外から内に向かっているので、依存関係のルールには違反していません。
...
UseCaseへの入力も、Entityではなく専用のDTOを使うことで同じメリットが得られます。
...
これを「ボイラープレートだ」「パフォーマンスが心配」と感じることもあります。

しかし、次のように考えられます。

- 変換処理は1箇所に集約される：UseCase内、またはAssembler/Mapperクラスに閉じる
- パフォーマンス影響は軽微：オブジェクト生成のコストは、DBアクセスやネットワーク通信に比べれば誤差
- 変換処理がドキュメントになる：何を外部に公開しているかが明示的になる
...
「依存関係が正しければOK」というのは最低限のルールです。

DTOを使うことで得られるのは、各層が「自分の責務だけ考えればいい」という状態です。

- Controller：「DTOをJSONにする」だけ考える
- UseCase：「Entityを操作してDTOを返す」だけ考える
- Entity：「外部からどう使われるか」を考えなくていい

「考える範囲が減る」というのは、言い換えれば認知負荷の軽減です。コードを書くときも、読むときも、レビューするときも、楽になります。

依存関係のルールを守るのは前提として、DTOによる境界の明確化は「やっておいて損はない」投資だと思います。
