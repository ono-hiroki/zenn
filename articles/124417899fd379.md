...
# Terralith to Terragrunt 実践ガイド

## はじめに

このガイドは、Terragrunt公式ドキュメントの「[Terralith to Terragrunt](https://terragrunt.gruntwork.io/docs/guides/terralith-to-terragrunt/)」を実践した記録です。

**Terralith**とは「Terraform + Monolith」の造語で、単一のStateファイルで大規模なインフラを管理している状態を指します。この構成の課題を段階的に解消し、スケーラブルなTerragrunt構成へ移行する過程を解説します。

### 構築するアプリケーション

猫画像を投票するWebアプリケーション「Best Cat」を題材に以下のAWSリソースをデプロイします：
