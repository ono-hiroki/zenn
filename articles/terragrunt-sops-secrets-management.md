---
title: "Terragrunt + SOPS でシークレットを安全に Git 管理する方法"
emoji: "🔐"
type: "tech"
topics: ["Terragrunt", "SOPS", "Terraform", "AWS", "IaC"]
published: false
---

# Terragrunt + SOPS でシークレットを安全に Git 管理する

## 課題

IaC でシークレットをどう管理するか？

- `.tfvars` に平文で書く → セキュリティリスク
- 環境変数で渡す → CI/CD が複雑に
- 外部シークレット管理ツール → 学習コスト

## 解決策

**SOPS** で暗号化して Git 管理し、**Terragrunt** の `sops_decrypt_file` で復号する。

```
secrets.yaml (暗号化済み)
        │
        ▼ Terragrunt: sops_decrypt_file()
        │
        ▼ AWS KMS で復号
        │
    Terraform に渡す
```

## 最小構成

```
live/
├── .sops.yaml              # SOPS 設定
└── dev/
    └── secrets/
        ├── terragrunt.hcl  # sops_decrypt_file() を使用
        └── secrets.yaml    # 暗号化済み
```

## セットアップ

### 1. SOPS インストール

```bash
brew install sops
```

### 2. .sops.yaml 作成

```yaml
creation_rules:
  - path_regex: \.yaml$
    kms: arn:aws:kms:ap-northeast-1:ACCOUNT_ID:alias/YOUR_KEY
```

### 3. シークレットファイル作成 & 暗号化

```yaml
# secrets.yaml
db_password: "secret123"
api_key: "key456"
```

```bash
sops --encrypt --in-place secrets.yaml
```

### 4. Terragrunt で使用

```hcl
locals {
  secrets = yamldecode(sops_decrypt_file("${get_terragrunt_dir()}/secrets.yaml"))
}

inputs = {
  db_password = local.secrets.db_password
  api_key     = local.secrets.api_key
}
```

## 日常操作

```bash
# 編集（復号→編集→再暗号化が自動）
sops secrets.yaml

# 確認
sops --decrypt secrets.yaml
```

## ポイント

- **キー名は平文**、値だけ暗号化 → diff が見やすい
- **Git で履歴管理**できる
- **Terragrunt ネイティブ対応**で追加ツール不要
- 環境ごとに異なる KMS キーで**権限分離**可能
